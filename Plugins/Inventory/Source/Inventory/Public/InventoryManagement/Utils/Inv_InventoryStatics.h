...
public:
	...
	static void ForEach2D(TArray<T>& Array, int32 Index, const FIntPoint& Ranged2D, int32 GridColumns, const FuncT& Function);
};

template <typename T, typename FuncT>
void UInv_InventoryStatics::ForEach2D(TArray<T>& Array, int32 Index, const FIntPoint& Ranged2D, int32 GridColumns, const FuncT& Function)
{
	for (int32 j = 0; j < Ranged2D.Y; ++j)
	{
		for (int32 i = 0; i < Ranged2D.X; ++i)
		{
